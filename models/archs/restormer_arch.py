...
class OverlapPatchEmbed(nn.Layer):
    def __init__(self, in_c=3, embed_dim=48, bias=False):
        ...
        self.proj = nn.Conv2D(in_c, embed_dim, kernel_size=3, stride=1, padding=1, bias_attr=bias)
...
class PixelUnshuffle(nn.Layer):
    def __init__(self, scale):
        super(PixelUnshuffle, self).__init__()
        self.scale = scale
    def forward(self, x):
        return pixel_unshuffle(x, self.scale)
...
class Downsample(nn.Layer):
    def __init__(self, n_feat):
        ...
        self.body = nn.Sequential(nn.Conv2D(n_feat, n_feat//2, kernel_size=3, stride=1, padding=1, bias_attr=False),
                                  PixelUnshuffle(2))
...
class Restormer(nn.Layer):
    def __init__(self, 
        ...
        self.reduce_chan_level3 = nn.Conv2D(int(dim*2**3), int(dim*2**2), kernel_size=1, bias_attr=bias)
        self.decoder_level3 = nn.Sequential(*[TransformerBlock(dim=int(dim*2**2), num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(num_blocks[2])])


        self.up3_2 = Upsample(int(dim*2**2)) ## From Level 3 to Level 2
        self.reduce_chan_level2 = nn.Conv2D(int(dim*2**2), int(dim*2**1), kernel_size=1, bias_attr=bias)
        ...
        if self.dual_pixel_task:
            self.skip_conv = nn.Conv2D(dim, int(dim*2**1), kernel_size=1, bias_attr=bias)
        ###########################
            
        self.output = nn.Conv2D(int(dim*2**1), out_channels, kernel_size=3, stride=1, padding=1, bias_attr=bias)
    ...
    def forward(self, inp_img):
        ...
        inp_dec_level3 = paddle.concat([inp_dec_level3, out_enc_level3], 1)
        inp_dec_level3 = self.reduce_chan_level3(inp_dec_level3)
        out_dec_level3 = self.decoder_level3(inp_dec_level3) 

        inp_dec_level2 = self.up3_2(out_dec_level3)
        inp_dec_level2 = paddle.concat([inp_dec_level2, out_enc_level2], 1)
        inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2)
        out_dec_level2 = self.decoder_level2(inp_dec_level2) 

        inp_dec_level1 = self.up2_1(out_dec_level2)
        inp_dec_level1 = paddle.concat([inp_dec_level1, out_enc_level1], 1)
